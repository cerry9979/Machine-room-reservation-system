...
void Student::CheckAllSSub()
{
	...
	{
		cout << i + 1 << "、";
		cout << "预约日期：周" << file.M_SO[i]["时间"];
		cout << "时段：" << (file.M_SO[i]["时间段"] == "1" ? "上午" : "下午");
		cout << "学号：" << (file.M_SO[i]["学生学号"]);
		cout << "姓名：" << (file.M_SO[i]["学生姓名"]);
		cout << "班级：" << file.M_SO[i]["选择班级"];
		string state = "状态";
		//0—取消预约	1—审核中
		//2—已预约		-1—预约失败
		if (file.M_SO[i]["审核状态"] == "1")
		{
			state += "审核中";
		}
		else if (file.M_SO[i]["审核状态"] == "2")
		{
			state += "预约成功";
		}
		else if (file.M_SO[i]["审核状态"] == "-1")
		{
			state += "未通过审核，预约失败";
		}
		else
